...
class PlayGolf(object):
    ...
    def leaderboard(self):
        ...
        order = list(np.unique(scores, return_inverse=True)[1])
        results = list(zip(order, names, scores))
        df_results = [(order+1, name, total) for order, name, total in results]
        sorted_df_results = sorted(df_results, key=lambda x: x[0])

        # for order, (name, total) in enumerate(sorted_results):
        #     print('{}. {}, Score: {}'.format(order+1, name, total))

        df = pd.DataFrame(sorted_df_results, columns=['Position', 'Name', 'Total'])
